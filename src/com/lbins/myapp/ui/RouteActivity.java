...
import com.lbins.myapp.adapter.BusResultListAdapter;
...
public class RouteActivity extends BaseActivity implements OnMapClickListener,
	...
	private String mCurrentCityName = "潍坊";
	...
	private String lat_company;//终点
	private String lng_company;//终点
	...
	protected void onCreate(Bundle bundle) {
		...
		lat_company = getIntent().getExtras().getString("lat_company");
		lng_company = getIntent().getExtras().getString("lng_company");
		if(!StringUtil.isNullOrEmpty(MeirenmeibaAppApplication.latStr) && !StringUtil.isNullOrEmpty(MeirenmeibaAppApplication.lngStr)){
			mStartPoint = new LatLonPoint(Double.valueOf(MeirenmeibaAppApplication.latStr), Double.valueOf(MeirenmeibaAppApplication.lngStr));//起点，116.335891,39.942295
			mStartPoint_bus = new LatLonPoint(Double.valueOf(MeirenmeibaAppApplication.latStr), Double.valueOf(MeirenmeibaAppApplication.lngStr));//起点，116.335891,39.942295
		}
		if(!StringUtil.isNullOrEmpty(lat_company) && !StringUtil.isNullOrEmpty(lng_company)){
			mEndPoint = new LatLonPoint(Double.valueOf(lat_company), Double.valueOf(lng_company));//起点，116.335891,39.942295
			mEndPoint_bus = new LatLonPoint(Double.valueOf(lat_company), Double.valueOf(lng_company));//起点，116.335891,39.942295
		}
		if(!StringUtil.isNullOrEmpty(MeirenmeibaAppApplication.locationCityName)){
			mCurrentCityName = MeirenmeibaAppApplication.locationCityName;
		}
...
//		getIntentData();
	...
	public void searchRouteResult(int routeType, int mode) {
		if (mStartPoint == null) {
			showMsg(mContext,"起点未设置");
			return;
		}
		if (mEndPoint == null) {
			showMsg(mContext, "终点未设置");
		}
		showProgressDialog();
		...
		if (errorCode == 1000) {
			if (result != null && result.getPaths() != null) {
				if (result.getPaths().size() > 0) {
					...
					showMsg(mContext,"对不起，没有搜索到相关数据！");
				}
			} else {
				showMsg(mContext, "对不起，没有搜索到相关数据！");
			}
		} else {
			showMsg(this.getApplicationContext(), "对不起，没有搜索到相关数据！");
		}
	}
