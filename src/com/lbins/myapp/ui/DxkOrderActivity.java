 ...
 * 定向卡支付
 */
public class DxkOrderActivity extends BaseActivity implements View.OnClickListener,Runnable  {
    private TextView title;

    DxkMoneyObj dxkMoneyObj;//定向卡充值金额对象c
...
//                        updateMineOrder();
                ...
                case SDK_CHECK_FLAG: {
                    Toast.makeText(DxkOrderActivity.this, "检查结果为：" + msg.obj,
                            Toast.LENGTH_SHORT).show();
                    break;
                }
                default:
...
//                                        payLingqian();
//                                    }else {
//                                        showMsg(DxkOrderActivity.this, "支付出现问题，请稍后！");
//                                    }
//                                }else{
//                                    showMsg(DxkOrderActivity.this, "支付出现问题，请稍后！");
//                                }
...
//                                Toast.makeText(DxkOrderActivity.this, R.string.order_error_three, Toast.LENGTH_SHORT).show();
//                                finish();
//                            }else if(data.getCode() == 3){
//                                Toast.makeText(DxkOrderActivity.this, R.string.order_error_four, Toast.LENGTH_SHORT).show();
//                            }
//                            else {
//                                Toast.makeText(DxkOrderActivity.this, R.string.order_error_one, Toast.LENGTH_SHORT).show();
//                                finish();
//                            }
//                        } else {
//                            Toast.makeText(DxkOrderActivity.this, R.string.order_error_one, Toast.LENGTH_SHORT).show();
...
//    }
    ...
    //清空购物车
    void deleteCart(){
        DBHelper.getInstance(DxkOrderActivity.this).deleteShopping();
                    ...
                    public void onResponse(String s) {
                        if (StringUtil.isJson(s)) {
                            ...
                            if (data.getCode() == 200) {
                                ...
                                // 启动一个线程
                                new Thread(DxkOrderActivity.this).start();
                            } else {
                                Toast.makeText(DxkOrderActivity.this, R.string.order_error_one, Toast.LENGTH_SHORT).show();
                                finish();
                            }
    ...
    public void getDxkMoney(){
        StringRequest request = new StringRequest(
                Request.Method.POST,
                InternetURL.GET_DXK_CHONGZHI_MONEY,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String s) {
                        if (StringUtil.isJson(s)) {
                            DxkMoneyObjData data = getGson().fromJson(s, DxkMoneyObjData.class);
                            if (data.getCode() == 200) {
                                dxkMoneyObj = data.getData();
                                if(dxkMoneyObj != null){
                                    String package_money = dxkMoneyObj.getMoney();//金额
                                    if(!StringUtil.isNullOrEmpty(package_money)){
                                        count_money.setText(getResources().getString(R.string.countPrices) + package_money);
                                    }else {
                                        showMsg(DxkOrderActivity.this, "支付出现问题，请稍后！");
                                    }
                                }else{
    ...
    void updateMineOrder(){
        StringRequest request = new StringRequest(
                Request.Method.POST,
                InternetURL.UPDATE_ORDER_DXK_TOSERVER,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String s) {
                        if (StringUtil.isJson(s)) {
